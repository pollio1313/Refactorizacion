...
function setupFormHandler() 
{
    ...
    form.addEventListener('submit', async e => 
    {
        ...
        try 
        {
            if (relation.id)                                                        //cambie este if para q no entre el duplicado
            {
            ...
            else 
            {
                ...
                const duplicate = existingRelations.some(rel => 
                    rel.student_id == relation.student_id && rel.subject_id == relation.subject_id
                );

                if (duplicate) {
                    showRelationMessage('Ya existe una inscripción para ese estudiante en esa materia.', true);
                    return;
                }
...
//funcion para mostrar elc artel de error

function showRelationMessage(msg, isError = true, duration = 2500) {
